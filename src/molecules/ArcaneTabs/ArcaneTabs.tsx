...
export const ArcaneTabs: React.FC<ArcaneTabsProps> = ({
  ...
  return (
    <div className={`flex items-center gap-2 p-1.5 rounded-2xl bg-slate-100 dark:bg-cyberdark border border-slate-200 dark:border-cyberborder font-mono text-xs overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden max-w-full ${className}`}>
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`px-4 py-2 rounded-xl font-bold flex items-center gap-2 whitespace-nowrap transition-all duration-200 select-none ${
              isActive
                ? activeBg
          ...
          >
            ...
            <span>{tab.label}</span>
            {tab.dot && <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0" />}
            {tab.badge && (
